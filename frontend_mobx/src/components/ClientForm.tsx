...
const ClientForm: React.FC<{ onClientAdded: () => void }> = observer(({ onClientAdded }) => {
  ...
  const [newSubscription, setNewSubscription] = useState({
    clientId: 0, // clientId будет установлен после добавления клиента
    status: "Активен",
    startDate: "",
    endDate: "",
  });
  ...
  const isValidClient = () => {
    return (
      Object.values(newClient).every((val) => val !== "") &&
